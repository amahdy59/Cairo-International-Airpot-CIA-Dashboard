...
// 1. Remove `<Hero activeTab={activeTab} setActiveTab={setActiveTab} language={language} />`
code = code.replace(
  /\s*<Hero activeTab=\{activeTab\} setActiveTab=\{setActiveTab\} language=\{language\} \/>/,
  ``
);

// 2. Remove the Hero component completely
const heroStart = code.indexOf('function Hero(');
if (heroStart !== -1) {
  const heroEnd = code.indexOf('</section>\n  );\n}', heroStart) + '</section>\n  );\n}'.length;
  code = code.substring(0, heroStart) + code.substring(heroEnd);
}

// 3. Update Header component definition
code = code.replace(
  /function Header\(\{[\s\S]*?\}\) \{[\s\S]*?return \([\s\S]*?<\/header>\n  \);\n\}/,
  `function Header({
  language,
  setLanguage,
  theme,
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={\`flex min-w-0 items-center justify-center gap-2 rounded-lg px-3 py-2 sm:px-4 sm:py-2 text-sm font-semibold transition \${isActive ? "bg-[#3154D4] dark:bg-primary text-white shadow-sm" : "text-muted-foreground hover:bg-secondary/60 hover:text-foreground"}\`}
                >
