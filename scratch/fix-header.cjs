...
code = code.replace(/IATA: CAI/g, "IATA: CIA");
code = code.replace(/CAI operations sample/g, "CIA operations sample");

// 2. Add activeTab and setActiveTab to Header props
code = code.replace(
  /function Header\(\{[\s\S]*?\}\) \{/,
  `function Header({
  language,
...
// 3. Rewrite Header return to include tabs
const headerStart = code.indexOf('return (', code.indexOf('function Header'));
const headerEnd = code.indexOf(');', headerStart) + 2;

const newHeaderReturn = `return (
...
code = code.substring(0, headerStart) + newHeaderReturn + code.substring(headerEnd);

// 4. Update App to pass activeTab to Header and remove Hero
code = code.replace(
...
// Remove `<Hero activeTab={activeTab} setActiveTab={setActiveTab} language={language} />`
code = code.replace(
  /\s*<Hero activeTab=\{activeTab\} setActiveTab=\{setActiveTab\} language=\{language\} \/>/,
  ``
);

// 5. Remove the Hero component completely
const heroStart = code.indexOf('function Hero(');
if (heroStart !== -1) {
  const heroEnd = code.indexOf('</section>', heroStart) + '</section>\n  );\n}'.length;
